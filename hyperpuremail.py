...
Downloads attachments named 'attachment.pdf' from specified Gmail sender/search terms and uploads to Google Drive
"""
...
class GmailGDriveAutomation:
    def __init__(self, credentials_path: str, gdrive_folder_id: Optional[str] = None, attachment_filter: Optional[str] = None):
        """
        ...
        Args:
            ...
            attachment_filter: Exact filename to filter attachments (e.g., 'attachment.pdf')
        """
        self.credentials_path = credentials_path
        self.gdrive_folder_id = gdrive_folder_id
        self.attachment_filter = attachment_filter.lower() if attachment_filter else None
    ...
    def search_emails(self, sender: str = "", search_term: str = "", 
        ...
        try:
            ...
            if sender:
                query_parts.append(f'from:"{sender}"')  # Quote sender to handle special characters
    ...
    def process_attachment(self, message_id: str, part: Dict, sender_info: Dict, 
        ...
        try:
            ...
            if self.attachment_filter:
                if filename.lower() != self.attachment_filter:
                    logger.info(f"[SKIPPED] Attachment {filename} does not match filter {self.attachment_filter}")
                    return False
            ...
            # Create folder structure in Drive (simplified: Gmail_Attachments -> search_term -> file_type)
    ...
    def run_automation(self, sender: str = "", search_term: str = "", 
                      days_back: int = 7, max_results: int = 50, attachment_filter: Optional[str] = None):
        """
        ...
        if attachment_filter:
            self.attachment_filter = attachment_filter.lower()
        
        logger.info("[START] Starting Gmail to Google Drive automation")
        logger.info(f"[CONFIG] Parameters: sender='{sender}', search_term='{search_term}', days_back={days_back}, attachment_filter='{attachment_filter}'")
...
def main():
    ...
    CONFIG = {
        'credentials_path': 'C:\\Users\\Lucifer\\Desktop\\New folder\\TBD\GRN\\Hyperpureautomation\\credentials.json',  # Path to your Google credentials JSON
        'gdrive_folder_id': '1euqxO-meY4Ahszpdk3XbwlRwvkfSlY8k',  # Optional: Google Drive folder ID to upload to
        'sender': 'noreply@hyperpure.com',  # Email address to search from
        'search_term': 'Hyperpure GRN',  # Keywords to search for
        'days_back': 7,  # How many days back to search
        'max_results': 1000,  # Maximum number of emails to process
        'attachment_filter': 'attachment.pdf'  # Only download attachments named 'attachment.pdf'
    }
